...
function ServerMultiSelect({servers,value,onChange}:{servers:Server[];value:string[];onChange:(ids:string[])=>void}){
  ...
  return <fieldset className="server-multi-select"><legend>SERVERS</legend><div>{servers.map((server,index)=><label key={server.id} className={value.includes(server.id)?'selected':''}><input type="checkbox" required={value.length===0&&index===0} checked={value.includes(server.id)} onChange={()=>toggle(server.id)}/><span><strong>{server.name}</strong><small>{serverStatus(server).label}</small></span></label>)}</div><small>{value.length?`${value.length} server${value.length===1?'':'s'} selected`:'Select at least one server'}</small></fieldset>
}
