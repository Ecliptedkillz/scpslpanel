...
function AdminEditorModal({form,setForm,servers,scopeServer,setScopeServer,permissionGroups,permissionOptions,
  ...
  const selected=form.serverAccess.find(x=>x.serverId===scopeServer)?.permissions??[]
  return <div className="modal-backdrop admin-modal-backdrop admin-modal-v2-backdrop" style={{padding:24,display:'grid',placeItems:'center'}}><form className="modal admin-modal admin-modal-v2" onSubmit={save} style={{width:'min(1100px, calc(100vw - 48px))',height:'min(820px, calc(100vh - 48px))',maxWidth:1100,maxHeight:820,display:'grid',gridTemplateRows:'auto minmax(0, 1fr) auto',overflow:'hidden',padding:0}}>
    ...
    <div className="admin-modal-body" style={{display:'block',minWidth:0,overflow:'auto'}}>
      ...
      <section className="admin-access-section" style={{display:'block',minHeight:0}}><div className="admin-section-title"><span>02</span><div><h3>Server access and permissions</h3><p>Select a server, then choose exactly what this administrator can do.</p></div></div><div className="admin-scope-grid" style={{display:'grid',gridTemplateColumns:'260px minmax(0, 1fr)',gap:14,width:'100%',height:'auto',minHeight:430}}>
        <aside className="admin-server-selector" style={{width:'100%',height:'auto',minHeight:430,overflow:'auto'}}><div className="selector-head"><span className="eyebrow">ASSIGNED SERVERS</span><b>{form.serverAccess.length}/{servers.length}</b></div>{servers.map(server=>{const assigned=form.serverAccess.some(x=>x.serverId===server.id);return <button type="button" className={`server-grant-row ${scopeServer===server.id?'active':''} ${assigned?'assigned':''}`} key={server.id} onClick={()=>assigned&&setScopeServer(server.id)}><input type="checkbox" checked={assigned} onClick={e=>e.stopPropagation()} onChange={()=>toggleServer(server.id)}/><span><strong>{server.name}</strong><small><i className={`server-dot ${server.state}`}/>{server.state}</small></span><b>{assigned?(scopeServer===server.id?'EDITING':'EDIT'):'NO ACCESS'}</b></button>})}</aside>
        <section className="admin-permission-selector" style={{display:'block',width:'100%',height:'auto',minHeight:430,overflow:'visible'}}>{scopeServer?<><div className="permission-selector-head"><div><span className="eyebrow">PERMISSIONS FOR</span><h3>{servers.find(x=>x.id===scopeServer)?.name}</h3><p>{selected.length} of {permissionOptions.length} selected</p></div><div className="permission-head-actions"><button type="button" onClick={()=>setScopedPermissions(permissionOptions.map(([value])=>value))}>SELECT ALL</button><button type="button" onClick={()=>setScopedPermissions([])}>CLEAR</button></div></div>
