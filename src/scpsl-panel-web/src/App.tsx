...
type ServerAssigned = { serverId:string; serverIds?:string[] }
const collapseServerAssignments = <T extends ServerAssigned>(items:T[]):T[] => {
  const grouped=new Map<string,T>()
  for(const item of items){
    const {serverId:_,serverIds:__,...value}=item
    const key=JSON.stringify(value)
    const existing=grouped.get(key)
    const ids=[...new Set([...(existing?.serverIds??(existing?[existing.serverId]:[])),...(item.serverIds??[item.serverId])])]
    grouped.set(key,{...(existing??item),serverId:ids[0]??'',serverIds:ids})
  }
...
function ServerMultiSelect({servers,value,onChange}:{servers:Server[];value:string[];onChange:(ids:string[])=>void}){
  const toggle=(id:string)=>onChange(value.includes(id)?value.filter(item=>item!==id):[...value,id])
  const allSelected=servers.length>0&&servers.every(server=>value.includes(server.id))
  return <fieldset className="server-multi-select"><legend>SERVERS</legend><div className="server-picker-head"><span><strong>Server scope</strong><small>{value.length?`${value.length} of ${servers.length} selected`:'Choose at least one server'}</small></span><div><button type="button" className={allSelected?'active':''} onClick={()=>onChange(servers.map(server=>server.id))}>SELECT ALL</button><button type="button" disabled={!value.length} onClick={()=>onChange([])}>CLEAR</button></div></div><div className="server-picker-grid">{servers.map((server,index)=>{const statusInfo=serverStatus(server);const selected=value.includes(server.id);return <label key={server.id} className={selected?'selected':''}><input type="checkbox" required={value.length===0&&index===0} checked={selected} onChange={()=>toggle(server.id)}/><span className="server-picker-icon"><ServerIcon size={15}/></span><span className="server-picker-copy"><strong>{server.name}</strong><small className={`server-picker-state ${statusInfo.className}`}><i/>{statusInfo.label}</small></span><span className="server-picker-check">✓</span></label>})}</div>{!servers.length&&<div className="server-picker-empty">No servers are available.</div>}</fieldset>
}
...
function ServerOverview({ server, setTab }: { server: Server; setTab: (tab: ServerTab) => void }) {
  ...
  return <section className="server-overview-grid">
    ...
    <div className="quick-grid">
      <button onClick={() => setTab('console')}><span className="quick-icon"><Terminal/></span><div><strong>Live console</strong><span>Watch output and run commands</span></div><ChevronRight/></button>
      <button onClick={() => setTab('players')}><span className="quick-icon"><Users/></span><div><strong>Players</strong><span>View connected players and moderation</span></div><ChevronRight/></button>
      <button onClick={() => setTab('plugins')}><span className="quick-icon"><Plug/></span><div><strong>Plugins</strong><span>Inspect EXILED and NWAPI assemblies</span></div><ChevronRight/></button>
      <button onClick={() => setTab('files')}><span className="quick-icon"><FileCode2/></span><div><strong>Files & configuration</strong><span>Open and edit files below the server directory</span></div><ChevronRight/></button>
    </div>
...
function BanModal({ close, saved, onError }: { close: () => void; saved: () => void; onError: (e: string) => void }) {
  ...
  useEffect(()=>{api<typeof players>('/players/global').then(setPlayers).catch(error=>onError(error.message))},[onError])
  const knownPlayers=[...players].sort((a,b)=>b.player.lastConnectedAt.localeCompare(a.player.lastConnectedAt))
  return <div className="modal-backdrop"><form className="modal small ban-modal" onSubmit={async e => { e.preventDefault(); try { await api('/bans', { method: 'POST', body: JSON.stringify({ playerId: target, reason, durationMinutes: duration || null }) }); saved() } catch (x) { onError(x instanceof Error ? x.message : 'Failed') } }}><div className="modal-head"><div><span className="eyebrow">MODERATION ACTION</span><h2>Issue ban</h2><p>Select a known player or enter an identifier manually.</p></div><button type="button" className="icon-button" onClick={close}><X/></button></div>{!!knownPlayers.length&&<label>KNOWN PLAYER<select value={knownPlayers.some(record=>record.player.userId===target)?target:''} onChange={e=>setTarget(e.target.value)}><option value="">Choose a player from server history…</option>{knownPlayers.map(record=><option value={record.player.userId} key={`${record.serverId}-${record.player.id}`}>{record.player.currentName} — {record.serverName} — {record.player.userId}</option>)}</select></label>}<label>USER ID / IP<input required value={target} onChange={e => setTarget(e.target.value)} placeholder="Steam ID, user ID, or IP address"/></label><label>REASON<textarea required value={reason} onChange={e => setReason(e.target.value)}/></label><label>DURATION IN MINUTES <small>(0 = permanent)</small><input type="number" min="0" value={duration} onChange={e => setDuration(Number(e.target.value))}/></label><div className="modal-actions"><button type="button" onClick={close}>CANCEL</button><button className="danger solid">ISSUE BAN</button></div></form></div>
}
...
type IntegrationSettings = {
  ...
  discordGameRoleGrants: {roleId:string;serverId:string;serverIds?:string[];groupName:string;priority:number;enabled:boolean;
...
const expandedIntegration = (settings:IntegrationSettings,serverIds:string[]):IntegrationSettings => ({
  ...
  // User badges are global. Keep one persisted row for backwards compatibility with
  // the existing API shape; the bridge deliberately ignores its serverId.
  customUserBadges:(settings.customUserBadges??[]).map(item=>({...item,serverId:serverIds[0]??item.serverId,serverIds:undefined})),
  customRoleBadges:expandServerAssignments(settings.customRoleBadges??[]),
})
...
function IngamePermissionsPage({servers,onError}:{servers:Server[];onError:(message:string)=>void}) {
  ...
  const load=useCallback(()=>api<IntegrationSettings>('/integrations').then(value=>{
    const loaded={...defaultIntegration,...value,discordGameRoleGrants:collapseServerAssignments(value.discordGameRoleGrants??[])}
  ...
  return <><PageTitle eyebrow="ACCESS CONTROL" title="In-game Permissions"><button className="primary" onClick={add}><Plus size={15}/> ADD ROLE</button></PageTitle>
    ...
    <form className="ingame-permissions-page" onSubmit={async event=>{event.preventDefault();try{await api('/integrations',{method:'PUT',body:JSON.stringify(expandedIntegration(settings,servers.map(server=>server.id)))});setPersistedSettings(settings);window.dispatchEvent(new CustomEvent('panel-success',{detail:'In-game permissions saved for all servers'}));setRoleModal(null);void loadHealth()}catch(error){onError(error instanceof Error?error.message:'Unable to save roles')}}}>
      ...
      {roles.map((role,index)=><article className={`panel ingame-role-editor ${roleModal?.index===index?'open':''} ${roleModal?.view?'readonly':''}`} key={index}><header><div><span className="eyebrow">{roleModal?.view?'ROLE DETAILS':`EDIT ROLE ${index+1}`}</span><h2>{role.groupName||'New in-game role'}</h2><p>Discord role {role.roleId||'not selected'} · Priority {role.priority}</p></div><div><label className="check-row"><input type="checkbox" checked={role.enabled} onChange={e=>update(index,{enabled:e.target.checked})}/> Enabled</label>{roleModal?.view&&<button type="button" className="edit" onClick={()=>setRoleModal({index,view:false})}><Pencil size={14}/> EDIT</button>}<button type="button" className="icon-button" onClick={closeEditor}><X size={18}/></button></div></header>
        <div className="form-row"><label>DISCORD ROLE<select value={role.roleId} onChange={e=>{const selected=guildRoles.find(item=>item.id===e.target.value);update(index,{roleId:e.target.value,...(!role.groupName&&selected?{groupName:selected.name.toLowerCase().replace(/[^a-z0-9]+/g,'-')}:{ }),...(!role.badgeText&&selected?{badgeText:selected.name}:{})})}}><option value="">Select a Discord role…</option>{role.roleId&&!guildRoles.some(item=>item.id===role.roleId)&&<option value={role.roleId}>Unknown role ({role.roleId})</option>}{guildRoles.map(item=><option value={item.id} key={item.id}>{item.name} · {item.id}</option>)}</select></label><ServerMultiSelect servers={servers} value={role.serverIds??[role.serverId]} onChange={serverIds=>update(index,{serverIds,serverId:serverIds[0]??''})}/><label>RANK NAME<input value={role.groupName} onChange={e=>update(index,{groupName:e.target.value.trim()})}/></label><label>PRIORITY<input type="number" value={role.priority} onChange={e=>update(index,{priority:Number(e.target.value)})}/></label></div>
...
function DonorManagementPageV2({servers,onError}:{servers:Server[];onError:(e:string)=>void}){
  ...
  const createDonor=()=>{const serverId=servers[0]?.id??'';setSettings({...settings,discordDonorRoleGrants:[...grants,{roleId:'',serverId,serverIds:serverId?[serverId]:[],tier:1,priority:0,enabled:true}]});setEditor({kind:'donor',index:grants.length,creating:true})}
  const createBadge=()=>{const serverIds=servers.map(server=>server.id);setSettings({...settings,customUserBadges:[...badges,{serverId:serverIds[0]??'',serverIds,steamId:'',badgeText:'',badgeColor:'silver'}]});setEditor({kind:'badge',index:badges.length,creating:true})}
  const createRoleBadge=()=>{const serverIds=servers.map(server=>server.id);setSettings({...settings,customRoleBadges:[...roleBadges,{roleId:'',serverId:serverIds[0]??'',serverIds,badgeText:'',badgeColor:'silver',priority:0,enabled:true}]});setEditor({kind:'roleBadge',index:roleBadges.length,creating:true})}
  ...
  const availablePlayers=badge?players.filter(record=>record.player.discordId).filter((record,index,all)=>all.findIndex(item=>item.player.userId===record.player.userId)===index):[]
  const roleBadgeModal=roleBadge&&<div className="modal-backdrop donor-modal-backdrop"><form className="modal donor-editor-modal" onSubmit={event=>{event.preventDefault();void save(false)}}><div className="modal-head"><div><span className="eyebrow">{editor?.creating?'CREATE':'EDIT'} ROLE BADGE</span><h2>Configure custom role badge</h2><p>Choose a Discord role, one or more servers, badge appearance, and matching priority.</p></div><button type="button" className="icon-button" onClick={close}><X size={18}/></button></div><div className="donor-modal-body"><label className="check-row"><input type="checkbox" checked={roleBadge.enabled} onChange={event=>updateRoleBadge(editor!.index,{enabled:event.target.checked})}/> Badge enabled</label><div className="form-row"><label>DISCORD ROLE<select required value={roleBadge.roleId} onChange={event=>updateRoleBadge(editor!.index,{roleId:event.target.value})}><option value="">Select a Discord role…</option>{roleBadge.roleId&&!roles.some(role=>role.id===roleBadge.roleId)&&<option value={roleBadge.roleId}>Unknown role ({roleBadge.roleId})</option>}{roles.map(role=><option value={role.id} key={role.id}>{role.name} · {role.id}</option>)}</select></label><ServerMultiSelect servers={servers} value={roleBadge.serverIds??[roleBadge.serverId]} onChange={serverIds=>updateRoleBadge(editor!.index,{serverIds,serverId:serverIds[0]??''})}/><label>BADGE TEXT<input required maxLength={64} value={roleBadge.badgeText} onChange={event=>updateRoleBadge(editor!.index,{badgeText:event.target.value})} placeholder="SUPPORTER"/></label><label>BADGE COLOR<select required value={roleBadge.badgeColor} onChange={event=>updateRoleBadge(editor!.index,{badgeColor:event.target.value})}>{gameBadgeColors.map(color=><option value={color} key={color}>{color.replaceAll('_',' ')}</option>)}</select></label><label>PRIORITY<input required type="number" value={roleBadge.priority} onChange={event=>updateRoleBadge(editor!.index,{priority:Number(event.target.value)})}/></label></div></div><div className="modal-actions"><button type="button" onClick={close}>CANCEL</button><button className="primary" disabled={busy}><Save size={15}/> {busy?'SAVING…':editor?.creating?'CREATE':'SAVE CHANGES'}</button></div></form></div>
  return <><PageTitle eyebrow="DISCORD INTEGRATION" title="Donors & Custom Badges"><button disabled={busy} onClick={()=>void save(true)}><RefreshCw size={15}/> SYNC NOW</button></PageTitle>
    ...
    <section className="ingame-permissions-page">{!!roleBadges.length&&<div className="ingame-role-grid">{roleBadges.map((item,index)=>{const role=roles.find(value=>value.id===item.roleId);const scope=item.serverIds??[item.serverId];return <article className={`panel ingame-role-card ${item.enabled?'':'disabled'}`} key={index}><header><div><h2>{item.badgeText||'Unnamed badge'}</h2><p>{role?.name??item.roleId??'No Discord role selected'}</p></div>{!item.enabled&&<span className="tag red">DISABLED</span>}</header><div className="role-card-stats"><div><small>SCOPE</small><strong>{scope.length===servers.length?'All servers':`${scope.length} server${scope.length===1?'':'s'}`}</strong></div><div><small>PRIORITY</small><strong>{item.priority}</strong></div><div><small>COLOR</small><strong>{item.badgeColor}</strong></div></div><footer><button className="edit" onClick={()=>setEditor({kind:'roleBadge',index,creating:false})}><Pencil size={14}/> EDIT</button><button className="danger" onClick={()=>void remove('roleBadge',index)}><Trash2 size={14}/> DELETE</button></footer></article>})}</div>}{!roleBadges.length&&<div className="empty-page"><Shield/><h2>No custom role badges</h2><p>Assign a badge to every linked player with a Discord role.</p><button className="primary" onClick={createRoleBadge}>CREATE FIRST ROLE BADGE</button></div>}</section>
    ...
    <section className="ingame-permissions-page">{!!badges.length&&<div className="ingame-role-grid">{badges.map((item,index)=>{const linked=players.find(record=>record.player.userId.split('@')[0]===item.steamId);return <article className="panel ingame-role-card" key={index}><header><div><h2>{item.badgeText||'Unnamed badge'}</h2><p>{linked?.player.discordDisplayName??linked?.player.currentName??item.steamId}</p></div><span className="tag">{item.badgeColor.replaceAll('_',' ').toUpperCase()}</span></header><div className="role-card-stats"><div><small>SCOPE</small><strong>All servers</strong></div><div><small>STEAM ID</small><strong>{item.steamId||'—'}</strong></div><div><small>COLOR</small><strong>{item.badgeColor}</strong></div></div><footer><button className="edit" onClick={()=>setEditor({kind:'badge',index,creating:false})}><Pencil size={14}/> EDIT</button><button className="danger" onClick={()=>void remove('badge',index)}><Trash2 size={14}/> DELETE</button></footer></article>})}</div>}{!badges.length&&<div className="empty-page"><Shield/><h2>No custom badges</h2><p>Create a bridge-managed badge for a linked player.</p><button className="primary" onClick={createBadge}>CREATE FIRST BADGE</button></div>}</section>
    {editor&&<div className="modal-backdrop donor-modal-backdrop" onMouseDown={event=>event.target===event.currentTarget&&close()}><form className="modal donor-editor-modal" onSubmit={event=>{event.preventDefault();void save(false)}}><div className="modal-head"><div><span className="eyebrow">{editor.creating?'CREATE':'EDIT'} {editor.kind==='donor'?'DONOR MAPPING':'USER BADGE'}</span><h2>{editor.kind==='donor'?'Configure donor role':'Configure custom badge'}</h2><p>{editor.kind==='donor'?'Choose a Discord role, server, donor tier, and priority.':'Choose a linked user, badge text, and badge color.'}</p></div><button type="button" className="icon-button" onClick={close}><X size={18}/></button></div><div className="donor-modal-body">
      {grant&&<><label className="check-row"><input type="checkbox" checked={grant.enabled} onChange={event=>updateGrant(editor.index,{enabled:event.target.checked})}/> Mapping enabled</label><div className="form-row"><label>DISCORD ROLE<select required value={grant.roleId} onChange={event=>updateGrant(editor.index,{roleId:event.target.value})}><option value="">Select a Discord role…</option>{grant.roleId&&!roles.some(role=>role.id===grant.roleId)&&<option value={grant.roleId}>Unknown role ({grant.roleId})</option>}{roles.map(role=><option value={role.id} key={role.id}>{role.name} · {role.id}</option>)}</select></label><ServerMultiSelect servers={servers} value={grant.serverIds??[grant.serverId]} onChange={serverIds=>updateGrant(editor.index,{serverIds,serverId:serverIds[0]??''})}/><label>DONOR TIER<input required type="number" min={1} value={grant.tier} onChange={event=>updateGrant(editor.index,{tier:Number(event.target.value)})}/></label><label>PRIORITY<input required type="number" value={grant.priority} onChange={event=>updateGrant(editor.index,{priority:Number(event.target.value)})}/></label></div></>}
      {badge&&<div className="form-row"><label>LINKED USER<select required value={badge.steamId} onChange={event=>updateBadge(editor.index,{steamId:event.target.value})}><option value="">Select a linked user…</option>{badge.steamId&&!availablePlayers.some(record=>record.player.userId.split('@')[0]===badge.steamId)&&<option value={badge.steamId}>{badge.steamId}</option>}{availablePlayers.map(record=>{const steam=record.player.userId.split('@')[0];return <option value={steam} key={record.player.id}>{record.player.discordDisplayName??record.player.currentName} · {steam}</option>})}</select></label><label>BADGE TEXT<input required maxLength={64} value={badge.badgeText} onChange={event=>updateBadge(editor.index,{badgeText:event.target.value})} placeholder="SUPPORTER"/></label><label>BADGE COLOR<select required value={badge.badgeColor} onChange={event=>updateBadge(editor.index,{badgeColor:event.target.value})}>{gameBadgeColors.map(color=><option value={color} key={color}>{color.replaceAll('_',' ')}</option>)}</select></label></div>}
...
function DonorManagementPage({servers,onError}:{servers:Server[];onError:(e:string)=>void}){
  ...
  return <><PageTitle eyebrow="DISCORD INTEGRATION" title="Donors & Custom Badges"><button onClick={()=>void save(true)} disabled={busy}><RefreshCw size={15}/> SAVE & SYNC NOW</button><button className="primary" onClick={add}><Plus size={15}/> ADD DONOR ROLE</button></PageTitle>
    ...
    <form className="ingame-permissions-page" onSubmit={event=>{event.preventDefault();void save(false)}}>
      {grants.map((grant,index)=>{const selected=roles.find(role=>role.id===grant.roleId);return <article className="panel ingame-role-editor open" key={index}><header><div><h2>{selected?.name??'New donor role'}</h2><p>Tier {grant.tier} · Priority {grant.priority}</p></div><div><label className="check-row"><input type="checkbox" checked={grant.enabled} onChange={event=>update(index,{enabled:event.target.checked})}/> Enabled</label><button type="button" className="danger" onClick={()=>setSettings({...settings,discordDonorRoleGrants:grants.filter((_,i)=>i!==index)})}><Trash2 size={14}/> REMOVE</button></div></header>
    ...
    <section className="ingame-permissions-page">
      {badges.map((badge,index)=>{const available=players.filter(record=>record.serverId===badge.serverId&&record.player.discordId);return <article className="panel ingame-role-editor open" key={index}><header><div><h2>{badge.badgeText||'New custom badge'}</h2><p>{badge.steamId||'No linked user selected'} · {badge.badgeColor}</p></div><button type="button" className="danger" onClick={()=>setSettings({...settings,customUserBadges:badges.filter((_,i)=>i!==index)})}><Trash2 size={14}/> REMOVE</button></header>
