...
function IngamePermissionsPage({servers,onError}:{servers:Server[];onError:(message:string)=>void}) {
  ...
  const remove=async(index:number)=>{if(await confirmation.ask('Delete in-game role?',`Remove ${roles[index].groupName||'this role'} from the pending configuration?`,'DELETE ROLE',true)){setSettings({...settings,discordGameRoleGrants:roles.filter((_,i)=>i!==index)});setRoleModal(null)}}
