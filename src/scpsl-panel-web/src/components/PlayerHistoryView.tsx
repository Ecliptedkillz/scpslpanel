...
export function PlayerHistoryView({
  ...
  return <section className="player-database">
    ...
    <div className="table-wrap"><table><thead><tr><th>PLAYER</th><th>STEAM ID</th><th>DISCORD ID</th><th>LAST CONNECTED</th><th>PLAYTIME</th><th/></tr></thead><tbody>{history.map(player => <tr key={player.id}><td><strong>{player.currentName}</strong><small>{player.nameHistory.length} known name{player.nameHistory.length === 1 ? '' : 's'}</small></td><td className="mono">{player.userId}</td><td className="mono">{player.discordId ?? 'Not linked'}</td><td>{ago(player.lastConnectedAt)}</td><td>{playtime(player.playtimeSeconds)}</td><td><button className="manage-button" onClick={() => setProfile(player)}>VIEW PROFILE</button></td></tr>)}</tbody></table></div>
    ...
    {profile && <div className="modal-backdrop"><div className="modal player-profile">
      <div className="modal-head"><div><span className="eyebrow">PLAYER PROFILE</span><h2>{profile.currentName}</h2><p className="mono">Steam: {profile.userId}</p><p className="mono">Discord: {profile.discordId ?? 'Not linked'}</p></div><button className="icon-button" onClick={() => setProfile(null)}><X/></button></div>
      <div className="profile-action-bar"><button onClick={() => recordAction('warning')}>ADD WARNING</button><button onClick={() => recordAction('watchlist')}>WATCHLIST</button><button onClick={() => recordAction('allowlist')}>ALLOWLIST</button><button onClick={() => void copyIdentifiers()}>COPY IDENTIFIERS</button></div>
